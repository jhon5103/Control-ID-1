...
db.transaction((tx) => {
  ...
  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS colaboradores (id INTEGER PRIMARY KEY AUTOINCREMENT, brand TEXT, model TEXT, hp INT);"
  );
});
...
const create = (obj) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      ...
      tx.executeSql(
        "INSERT INTO colaboradores (brand, model, hp) values (?, ?, ?);",
...
const update = (id, obj) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      //comando SQL modificável
      tx.executeSql(
        "UPDATE colaboradores SET brand=?, model=?, hp=? WHERE id=?;",
...
const findByBrand = (brand) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      //comando SQL modificável
      tx.executeSql(
        "SELECT * FROM colaboradores WHERE brand LIKE ?;",
        [brand],
        //-----------------------
        (_, { rows }) => {
